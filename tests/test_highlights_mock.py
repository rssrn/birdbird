...
class TestDetectHardwareEncoder:
    ...
    def test_encoder_available_and_works(self, mocker):
        ...
        mocker.patch("birdbird.highlights.subprocess.run", side_effect=[encoders_result, test_result])
        result = detect_hardware_encoder()

        assert result == "h264_qsv"

    def test_encoder_listed_but_test_fails(self, mocker):
        """Tries next encoder when test fails, returns None if all fail."""
        encoders_result = MagicMock(stdout="h264_qsv h264_vaapi", returncode=0)
        test_fail = MagicMock(returncode=1)

        mocker.patch("birdbird.highlights.subprocess.run", side_effect=[encoders_result, test_fail, test_fail])
        result = detect_hardware_encoder()
    ...
    def test_ffmpeg_not_found(self, mocker):
        ...
        mocker.patch("birdbird.highlights.subprocess.run", side_effect=FileNotFoundError)
        result = detect_hardware_encoder()
...
class TestGetVideoDuration:
    ...
    def test_valid_video(self, mock_video_capture, mocker):
        """Returns frames/fps for a valid video."""
        cap = mock_video_capture(fps=30.0, frame_count=300)

        mock_cv2 = mocker.patch("birdbird.highlights.cv2")
    ...
    def test_video_wont_open(self, mock_video_capture, mocker):
...
class TestBinarySearchEntry:
    ...
    def test_converges_to_correct_entry_point(self, mocker):
        ...
        mocker.patch("birdbird.highlights._detect_at_time", side_effect=detect_at_time)
...
class TestBinarySearchExit:
    ...
    def test_converges_to_correct_exit_point(self, mocker):
...
class TestFindBirdSegments:
    ...
    def test_with_known_first_bird(self, mock_video_capture, mocker):
        ...
        mocker.patch("birdbird.highlights._detect_at_time", return_value=True)
    ...
    def test_no_bird_detected(self, mock_video_capture, mocker):
        ...
        mocker.patch("birdbird.highlights._detect_at_time", return_value=False)
        segments = find_bird_segments(Path("test.avi"), detector)

        assert segments == []

    def test_video_too_short(self, mock_video_capture, mocker):
...
class TestExtractSegment:
    ...
    def test_software_encoder(self, mocker):
        """Software encoder builds correct ffmpeg command."""
        segment = Segment(clip_path=Path("clip.avi"), start_time=1.0, end_time=5.0)

        mocker.patch("birdbird.highlights.detect_hardware_encoder", return_value=None)
        mock_run = mocker.patch("birdbird.highlights.subprocess.run")
    ...
    def test_hardware_encoder(self, mocker):
        """Hardware encoder is used when available."""
        segment = Segment(clip_path=Path("clip.avi"), start_time=1.0, end_time=5.0)

        mocker.patch("birdbird.highlights.detect_hardware_encoder", return_value="h264_qsv")
        mock_run = mocker.patch("birdbird.highlights.subprocess.run")
        mock_run.return_value = MagicMock(returncode=0, stderr="")
    ...
    def test_hw_fails_falls_back_to_software(self, mocker):
    ...
    def test_optimize_web_flag(self, mocker):
...
class TestConcatenateSegments:
    ...
    def test_single_segment(self, tmp_path, mocker):
    ...
    def test_multiple_segments(self, tmp_path, mocker):
